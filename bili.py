...
class Bilibili():
    ...
    def search_movie(self, keyword):
        link = Bilibili.try_pgc_search(self, keyword)
        if not link is None:
            return Bilibili.get_cid(self, link)
        link = Bilibili.try_movie_search(self, keyword)
        if not link is None:
            return Bilibili.get_cid(self, link)
        return None

    def try_pgc_search(self, keyword):
    ...
    def get_cid(self, link):
        try:
            url = "https:" + link
            print(url)
            r = requests.get(
                url,
                headers = Bilibili.fake_headers(self))
            if r.status_code != 200:
                return
            pattern = "cid\=\"(.+?)\""
            pattern1 = "cid\=(.+?)&"
    ...
    def try_movie_search(self, keyword):
    ...
    def convert_danmu_2_json(self, response):
        danmus = []
    ...
    def get_danmu_by_cid(self, id):
        r = requests.get(str.format(BILIBILI_DANMU_URL, id))
        return Bilibili.convert_danmu_2_json(self, r.text)
